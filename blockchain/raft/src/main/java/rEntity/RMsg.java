...
public final class RMsg implements Serializable {
    /**
     * *************************************************************************************************************************
     * 消息类型：心跳消息 选举消息  被选举消息  日志消息   发送日志状态消息 ，错误消息(LEADER节点宕机后发送心跳消息接收，包含最新网络状态信息，如新的LEADER)
     * 消息属性：
     * 日志类型：          log_type             int
     * 任期：             term              int
     * 索引:             index              int
     * 发送消息的节点：    ori_node          RNode
     * 发送消息的节点类型：ori_node_type     int (在节点中定义)
     * 消息类型：         type              int
     * LEADER：          leader            RNode
     * 日志信息：         log               HashMap  -> 包含之前的所有index term
     * 日志状态消息：      log_state          boolean
     * 需要选举的LEADER： pre_leader         RNode
     * etc....
     * <p>
     * <p>
     * <p>
