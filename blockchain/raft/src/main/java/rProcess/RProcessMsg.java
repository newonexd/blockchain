...
public final class RProcessMsg extends Thread {
    ...
    public static void processMsg(RNode node, RMsg msg) throws IOException, InterruptedException {
        ...
        if (msg.getTerm() < node.getTerm()) {
            //说明日志中任期小于当前节点任期
            //返回错误消息
            SendMsg.send_msg(msg.getNode().getLocation(), RCrtRmsg.crtErrorMsg(node));
        }
                ...
                if (msg.getLog_flag()) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if ((sum + 1) > node.get_Connect_node_count() / 2) {
                            //发送提交日志
                            SendMsg.send_msg(location, RCrtRmsg.crtLogComMsg(node, index));
                        }
                    }
                } else {
                    int index = msg.getIndex();
                    RMsg rMsg = RCrtRmsg.crtHeartMsg(node);
                    SendMsg.send_msg(msg.getNode().getLocation(), rMsg.addHashMap(index - 1, node.getRecv_log().get(index - 2)));
                }
                break;
            }
        ...
        switch (msg.getLog_type()) {
            case Constant.R_HEARTMSG: {
                ...
                if (msg.getHashMap().size() == 0) {
                    //返回心跳响应消息
                    SendMsg.send_msg(msg.getLeader().getLocation(), RCrtRmsg.crtHeartResMsg(node));
                    break;
                }
                ...
                if (node.getRecv_log().get(index - 1).getTerm() == term) {
                    //返回日志匹配消息
                    SendMsg.send_msg(msg.getNode().getLocation(), RCrtRmsg.crtLogStateMsg(node, index, true));
                } else {
                    //返回日志不匹配消息
                    SendMsg.send_msg(msg.getNode().getLocation(), RCrtRmsg.crtLogStateMsg(node, index, false));
                }
                break;
            }
            case Constant.R_ELECTEMSG: {
                if (!node.getElect_flag()) {
                    SendMsg.send_msg(msg.getNode().getLocation(), RCrtRmsg.crtElectedMsg(node, msg));
